...
# @hijung - for inference (2023.10.31)
class CustomPadFill_2():
    def __init__(self, padding_token, config):
        self.config = config
        self.padding_token = padding_token
    ...
    def __call__(self, bs):
        ...
        for feature, file in bs:
            feature_len += [feature.shape[-1]]
            file_fns += [file]
        # max_feature_len = min(max(feature_len), self.config.mfcc_max_len)
        max_feature_len = max(feature_len)

        for (feature, _), current_feature_len in zip(bs, feature_len):
            features += [self.custom_feature_padding(feature, max_feature_len, current_feature_len).unsqueeze(0)]
        features = torch.cat(features, dim=0).transpose(-1,-2)
        ...
        return features, feature_len, file_fns
...
def split_dataset_1(
    ...
    # valid_dataset = CustomDataset_2(  # JH dataset_1-> dataset_2로 변경
    #     valid,
    #     config=config,
    #     # valid_yn = True # 이 부분 수정.
    # )
...
class CustomDataset_1(Dataset):
    ...
    def __init__(
        ...
        print(f"{'Valid' if self.valid_yn else 'Train'} Datset | Add Noise/Masking Augmentation : {self.config.use_augment}")
    ...
    def wav2image_tensor(self, path):
        ...
        # @hijung - add for cleansing (2023.11.02) - for only valid
        if self.valid_yn:
            if self.config.remove_noise:
                audio = remove_noise_data(audio)
        ...
        # @hijung- add noise  - for only train
        if (not self.valid_yn) and (self.config.use_augment):
            ...
            # @hijung - remove for sunhanmatt!!! (2023.11.03)
            # elif agument_num <= 8:
            #     # 23-11-02 소정 추가
            #     noise_path = [os.path.join('noise/', i) for i in os.listdir('noise')]
        ...
        # @hijung - for test log mel spectrogram
...
class CustomDataset_2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return mfccs, _file_path # feature : spectogram audio, trasncript : list contain tokens
