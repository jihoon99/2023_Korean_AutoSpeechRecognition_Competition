...
def get_criterion(config, vocab_output_size, pad_index, if_add_blank_id=False) -> nn.Module:

    if config.loss_fn == 'ctcloss':
        criterion = nn.CTCLoss(blank=pad_index, reduction=config.reduction, zero_infinity=True)
    elif config.loss_fn == 'nll':
        tmp = torch.ones(vocab_output_size)
        tmp[pad_index] = 0
        ...
        criterion = nn.CrossEntropyLoss(weight=tmp, reduction='sum')
