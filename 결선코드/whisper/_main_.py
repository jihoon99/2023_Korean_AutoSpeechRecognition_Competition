...
def inference(path, model, config, **kwargs):
    ...
    for i in glob(os.path.join(path, '*')): # glob이 도커에 안깔림...
        results.append(
            {
                'filename': i.split('/')[-1],
                'text': custom_oneToken_infer_for_testing(model, i, "_", config).replace("<eos>", "")
            }
        )
    return sorted(results, key=lambda x: x['filename'])
...
if __name__ == '__main__':
    ...
    # Parameters 
    args.add_argument('--version', type=str, default='train')          #POC, train, inferOnly
    args.add_argument("--model_fn", default='11/model')    # False
